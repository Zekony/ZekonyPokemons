...
@Composable
fun NoDamageToColumn(pokemonType: TypePokemonInfo) {
    if (!pokemonType.damageRelations.noDamageTo.isNullOrEmpty()) {
        Text(text = pokemonType.damageRelations.noDamageTo.first().javaClass.simpleName.separateWord()
            .replaceFirstChar { it.uppercase() },
            fontSize = 26.sp,
            fontWeight = FontWeight.SemiBold
        )
        pokemonType.damageRelations.noDamageTo.forEach {
            Row(
                modifier = Modifier
                    .fillMaxWidth()
